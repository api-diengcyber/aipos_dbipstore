...
class Penjualan_retail_model extends CI_Model
{
    ...
    function json($id_toko)
    {
        ...
        if ($this->userdata->level != 1) {
            $this->datatables->where('o.id_users', $this->userdata->id_users);
        }
    ...
    // get by between date 
    function get_by_between_date($id_toko, $tgl1 = '', $tgl2 = '', $pembayaran = '', $bank = '', $no_faktur = '', $imei = '')
    {
        ...
        return $this->db->get()->result();
        */
        $this->db->select('o.*,o.id_orders_2 as id_o, od.id_produk, u.first_name,u.last_name, (o.nominal) AS nominal, SUM(rjd.jumlah) AS jumlah_retur, m.nama AS nama_pembeli,pr.id_produk_2 as idp ,pr.barcode');
        $this->db->from('orders o');
        ...
        $this->db->join('orders_detail od', 'od.id_orders_2=o.id_orders_2 AND od.id_toko=o.id_toko', 'left');

        $this->db->join('retur_jual_detail rjd', 'rjd.id_orders_detail=od.id_orders AND rjd.id_toko=od.id_toko', 'left');
    ...
    function get_by_between_date_and_user($id_toko, $id_users, $tgl1 = '', $tgl2 = '', $pembayaran = '')
    {
        $kasir_cabang = $this->db->select('u.*')
            ...
            ->get()
            ->row();
        $this->db->select('o.*, u.first_name, u.last_name, (o.nominal) AS nominal, m.nama AS nama_pembeli,pr.barcode');
        $this->db->from('orders o');
        ...
        $this->db->or_where('o.id_sales', $id_users);
        $this->db->where_in('o.pembayaran', array('1', '2', '3', '4', '5', '6'));
        $this->db->group_by('o.no_faktur');
        return $this->db->get()->result();
    }
