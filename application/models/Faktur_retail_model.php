...
class Faktur_retail_model extends CI_Model
{
	...
	function get_by_id_toko($id_toko = '', $deadline = '', $status = '', $opsi = '')
	{
		...
		$this->db->join('supplier s', 'fr.id_supplier=s.id_supplier AND s.id_toko=fr.id_toko', 'left');
		$this->db->join('hutang h', 'fr.id_faktur=h.id_faktur AND h.id_toko=s.id_toko', 'left');
