...
// }
    ...
    {
        $data = $this->db->select('so.*,s.nama as nama_service,CONCAT(u.first_name, " ", u.last_name) as nama_mekanik')
            ->from('service_order so')
            ->join('service s', 's.id=so.id_service')
            ->join('users u', 'u.id_users=so.id_mekanik')
            ->where('so.id_toko', $id_toko)
            ->order_by('so.id', 'desc')
            ->get()
            ->result();
        return $data;
    ...
    {
        // $data_login = $this->Tampilan_retail_model->cek_login();
        $this->datatables->select('s.harga');
        $this->datatables->from('service s');
        $this->datatables->where('s.id_toko', $id_toko);
        $this->datatables->add_column('action', anchor(site_url('service/read/$1'), '<button type="button" class="btn btn-xs btn-success"><i class="ace-icon fa fa-check bigger-120"></i></button>') . "&nbsp;&nbsp;&nbsp;&nbsp;" . anchor(site_url('service/update/$1'), '<button type="button" class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button>') . "&nbsp;&nbsp;&nbsp;&nbsp;" . anchor(site_url('service/delete/$1'), '<button type="button" class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash bigger-120"></i></button>', 'onclick="javasciprt: return confirm(\'Are You Sure ?\')"'), 'id');
