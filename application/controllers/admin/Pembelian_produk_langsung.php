...
class Pembelian_produk_langsung extends AI_Admin
{
	...
	public function create($id_produk_2 = '')
	{
		...
		if ($id_produk_2 != null) {
			...
			// var_dump($parent);
		} elseif ($id_produk_2 = $this->input->post('id_produk')) {
			$parent = $this->Produk_retail_model->get_by_id($id_produk_2);
			$kode_produk = $parent->kode_produk;
		} else {
			$parent = null;
		...
		$data = array(
			...
			'button' => 'Tambah',
			'action' => site_url('admin/pembelian_produk_langsung/create_action'),
			'action2' => site_url('admin/pembelian_produk_langsung/create'),
		...
		$this->view('pembelian_produk_langsung/pembelian_produk_langsung_form', $data);
	}
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->input->post('id_parent')) {
				...
				$data_beli = array(
					...
					'tgl_expire' => $this->input->post('expired', TRUE),
					'pembayaran' => '1',
				...
				redirect(site_url('admin/pembelian_produk_langsung/create/' . $id_produk_2));
			} else {
				...
				$this->Stok_produk_model->insert($data_stok);
				$this->session->set_flashdata('message', 'Create Record Success');
				redirect(site_url('admin/pembelian_produk_langsung/create'));
			}
		}
	}
	...
	public function update($id)
	{
		...
		// 						  ->get();
		if ($row) {
			$row_sp = $this->db->select('sp.*, p.harga_satuan,p.id_supplier')
				->from('stok_produk sp')
