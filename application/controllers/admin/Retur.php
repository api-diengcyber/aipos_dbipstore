...
class Retur extends AI_Admin
{

    public function __construct()
    {
        parent::__construct();
        $this->models('Produk_retail_model, Stok_produk_model, Faktur_retail_model, Supplier_retail_model');
        // $this->load->model('Pengaturan_transaksi_model');
    }

    /*public function index()
       {
           $active = array('active_retur_pembelian' => 'active');
           ...
           $data = array();
           $data['data_retur'] = $this->db->select('r.*, f.no_faktur, p.nama_produk')
                                          ->from('retur r')
                                          ->join('faktur_retail f', 'r.id_faktur=f.id_faktur AND f.id_toko="'.$this->userdata->id_toko.'"', 'left')
                                          ->join('produk_retail p', 'r.id_produk=p.id_produk_2 AND p.id_toko="'.$this->userdata->id_toko.'"', 'left')
           ...
           $this->view('retur/retur_list', $data);
       }*/
    ...
    public function index()
    {
        ...
        $data = array();
        $data['active_retur_pembelian'] = 'active';
    ...
    {
        $data_retur = $this->db->select('r.*, f.no_faktur, f.pembayaran, s.nama_supplier, h.hutang, h.kurang')
            ->from('retur r')
            ->join('faktur_retail f', 'r.id_faktur=f.id_faktur AND f.id_toko=r.id_toko')
            ->join('supplier s', 's.id_supplier=f.id_supplier AND s.id_toko=r.id_toko')
        ...
        if ($data_retur) {
            ...
            $this->view('retur/retur_read', $data);
        } else {
    ...
    public function produk_ajax($id_faktur)
    {
        ...
        $res_produk = $this->db->select('p.*, p.jumlah AS stok_sisa, pr.nama_produk, sp.satuan AS nama_satuan')
            ->from('pembelian p')
        ...
        foreach ($res_produk as $key):
            ...
            $html .= '<tr>
                        ...
                        <td style="padding:0px;background-color:yellow;"><input type="text" name="jumlah[' . $key->id_pembelian . ']" class="form-control text-right jumlah" data-id="' . $key->id_pembelian . '" value="1" style="border:none!important;background-color:yellow;color:#000;" readonly></td>
                        <td class="text-right"><span class="subtotal" data-id="' . $key->id_pembelian . '">' . number_format($key->total_bayar, 0, ',', '.') . '</span></td>
                    </tr>';
        endforeach;
        $data = array(
            "status" => "ok",
    ...
    public function create()
    {
        $data = array(
            ...
            'action' => site_url('admin/retur/create_action'),
            'id' => set_value('id'),
        ...
        $this->view('retur/retur_form', $data);
    }
    ...
    public function create_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $row_faktur = $this->db->select('fr.*, h.id, h.kurang')
            ...
            foreach ($jumlah as $id_pembelian => $value) {
                $row_pembelian = $this->db->select('p.*, p.jumlah AS stok')
                    ->from('pembelian p')
                ...
                if ($row_pembelian && $value > 0) {
                    ...
                    $pot = ($row_pembelian->harga_satuan * $value);
                    $data_update = array(
                        'jumlah' => $row_pembelian->stok - $value,
                        'total_bayar' => $row_pembelian->total_bayar - $pot,
                    );
                    $this->db->where('id', $row_pembelian->id);
                    $this->db->update('pembelian', $data_update);

                    $row_hutang = $this->db->where('id_toko', $this->userdata->id_toko)->where('id_faktur', $id_faktur)->get('hutang')->row();
                    if ($row_hutang) {
                        $bayar = $row_hutang->bayar;
                        if ($row_hutang->bayar >= $pot) {
                            $bayar = $row_hutang->bayar - $pot;
                        }
                        $kurang = $row_hutang->kurang;
                        if ($row_hutang->kurang >= $pot) {
                            $kurang = $row_hutang->kurang - $pot;
                        } // 0
                        $data_update = array(
                            'hutang' => $row_hutang->hutang - $pot,
                            'bayar' => $bayar,
                            'kurang' => $kurang,
                        );
                        $this->db->where('id_toko', $this->userdata->id_toko);
                        $this->db->where('id_faktur', $id_faktur);
                        $this->db->update('hutang', $data_update);
                    }
                }
            }
            if ($total > 0) {
    ...
    public function update($id)
    {
        ...
        if ($row) {
            $data = array(
                'active_retur_pembelian' => 'active',
                'action' => site_url('admin/retur/update_action'),
