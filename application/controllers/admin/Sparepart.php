...
class Sparepart extends AI_Admin
{
	...
	public function index($id_produk_2 = '') // parent id
	{
		...
		$this->view('sparepart/sparepart_list', $data);
	...
	public function json($id_produk_2 = '', $id_users = '') // parent id
	{
		...
		echo $this->Produk_retail_model->json($this->userdata->id_toko, $id_produk_2, $id_users, $kode);
	}
	...
	public function read($id)
	{
		...
		$row_stok = $this->get_stok($row->id_produk_2, $this->userdata->id_toko);
		$stok = 0;
		if (!empty ($row_stok)) {
			$stok = $row_stok->stok;
		}
	...
	public function get_stok($data_id_produk, $id_toko)
	{
		...
		$this->Mutasi_stok_model->query_stok_mutasi($this->db, $id_toko, $this->userdata->id_users, 'pr.id_produk_2');
		$this->db->where('pr.id_toko', $id_toko);
	...
	public function create($id_produk_2 = '')
	{
		...
		$data = array(
			...
			'button' => 'Tambah',
			'action' => site_url('admin/sparepart/create_action'),
			...
			'tgl_expire' => set_value('tgl_expire', date('d-m-Y')),
		);
		$this->view('sparepart/sparepart_form', $data);
	}
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->input->post('id_parent')) {
				...
				$data_beli = array(
					...
					'tgl_expire' => $this->input->post('expired'),
					'pembayaran' => '1',
					'harga_satuan' => $harga_beli,
					'id_supplier' => $this->input->post('supplier'),
					'jumlah' => 1,
					'total_bayar' => $harga_beli,
				);
				...
				redirect(site_url('admin/sparepart/index/' . $id_produk_2));
			} else {

				// $barcode = $this->db->select('p.*')
				// 			->from('produk_retail p')
				...
				redirect(site_url('admin/sparepart'));
			}
		}
	}
	...
	public function update($id)
	{
		...
		// 						  ->get();
		if ($row) {
			$row_sp = $this->db->select('sp.*, p.harga_satuan,p.id_supplier,p.tgl_expire')
				->from('stok_produk sp')
			...
			$data = array(
				...
				'nama_modul' => $this->userdata->nama_modul,
				'button' => 'Simpan',
				'action' => site_url('admin/sparepart/update_action'),
				...
				'data_kategori' => $data_kategori,
				'parent' => set_value($row->parent_id),
				'tgl_expire' => set_value('tgl_expire', $row_sp->tgl_expire),
			);
			// var_dump($row_sp);
			$this->view('sparepart/sparepart_form', $data);
		} else {
