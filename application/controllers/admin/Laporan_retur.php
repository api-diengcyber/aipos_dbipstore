...
class Laporan_retur extends AI_Admin
{
  ...
  public function index($page = '')
  {
    ...
    $this->db->select('r.*,pr.barcode,pr.nama_produk,u.*');
    $this->db->from('retur_jual r');
    $this->db->join('retur_jual_detail rd', 'rd.id_retur=r.id_retur', 'left');
    $this->db->join('produk_retail pr', 'pr.id_produk_2=rd.id_produk', 'left');
    $this->db->join('users u', 'u.id_users=r.id_users', 'left');
    if (!empty($no_return)) {
      $this->db->where('r.no_retur', $no_return);
    }
    if (!empty($dari) && !empty($sampai)) {
      $this->db->where('r.tgl BETWEEN ' . "'" . $dari . "'" . ' AND ' . "'" . $sampai . "'");
    }
    if (!empty($jenis_retur)) {
      $this->db->where('r.retur', $jenis_retur);
    }
    if (!empty($user)) {
    ...
    $this->db->where('r.id_toko', $this->userdata->id_toko);
    $retur = $this->db->get()->result();
    ...
    $data = [
      ...
      'no_return' => $no_return,
      'sampai' => $sampai,
      'jenis_retur' => set_value('jenis_retur', $jenis_retur),
    ];

    // var_dump($retur);
    $this->view('laporan/laporan_retur_jual2', $data);
