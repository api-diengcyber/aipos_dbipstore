...
class Laporan_retail extends AI_Admin
{
	...
	public function penjualan()
	{
		...
		if (!empty($this->input->post('kasir'))) {
			...
			$data['contents'] = $this->Penjualan_retail_model->get_by_between_date_and_user($this->userdata->id_toko, $this->input->post('kasir', true), $s_awal_periode, $s_akhir_periode, $this->input->post('pembayaran', true), $data['bank'], $data['no_faktur'], $data['imei']);
		} else {
			$data['pembayaran'] = $this->input->post('pembayaran', true);
			...
			$data['no_faktur'] = $this->input->post('no_faktur', true);
			$data['imei'] = $this->input->post('imei', true);
			$data['contents'] = $this->Penjualan_retail_model->get_by_between_date($this->userdata->id_toko, $s_awal_periode, $s_akhir_periode, $data['pembayaran'], $data['bank'], $data['no_faktur'], $data['imei']);
