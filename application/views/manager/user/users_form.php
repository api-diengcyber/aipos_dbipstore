          ...
          <div class="col-md-6">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" name="email" id="email" placeholder="Email"
                value="<?php echo $email; ?>" <?php echo $ro_email; ?> />
            </div>
            <div class="form-group">
              <label for="varchar">Nama Depan
            ...
            <div class="form-group">
              <label for="varchar">Nama Belakang
            ...
            <div class="form-group">
              <label for="varchar">Password
                <?php if ($button == 'Update') { ?> Baru
                <?php }
                ; ?>
                <?php echo form_error('password_baru') ?>
              </label>
              <input type="password" class="form-control" name="password_baru" id="password_baru" placeholder="Password <?php if ($button == 'Update') { ?> Baru <?php }
              ; ?>" value="<?php echo $password_baru; ?>" />
              <?php if ($button == 'Update') { ?>
                <font style="color:red;">* diisi jika ingin mengubah password</font>
              <?php }
              ; ?>
            </div>
            <div class="form-group">
              <label for="varchar">Level
                <?php echo form_error('level') ?>
              </label>
              <select name="level" id="level" class="form-control">
                <?php foreach ($pil_level as $p_level) { ?>
                  <option value="<?php echo $p_level->id; ?>" <?php if ($level == $p_level->id) {
                       echo 'selected';
                     } ?>>
                    <?php echo $p_level->level; ?>
                  </option>
                <?php } ?>
              </select>
            </div>
            <div class="form-group">
              <label for="varchar">CABANG
              ...
              <select name="cabang" id="cabang" class="form-control">
                ...
                if ($this->userdata->level != 1) {
                  ...
                  <option value="0" <?php if ($cabang == 0) {
                    echo 'selected';
                  } ?>>PUSAT</option>
                  <?php
                ...
                <?php foreach ($pil_cabang as $p_cabang) { ?>
                  <option value="<?php echo $p_cabang->id; ?>" <?php if ($cabang == $p_cabang->id) {
                       echo 'selected';
                     } ?>>
                    <?php echo $p_cabang->nama; ?>
                  </option>
                <?php } ?>
              </select>
            </div>
