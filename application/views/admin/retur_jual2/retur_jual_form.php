            ...
            <div class="row">
              <div class="col-md-4">
                <div class="form-group">
                  <label for="varchar">CARI IMEI / NAMA BARANG
                    <?php echo form_error('no_retur') ?>
                  </label>
                  <input type="text" class="form-control" name="cari_barang" id="cari_barang"
                    placeholder="CARI IMEI/NAMA BARANG" />
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
                  <label for="varchar">Tanggal
                    <?php echo form_error('tanggal') ?>
                  </label>
                  <input type="text" class="form-control" name="tanggal" id="datepicker1" value="<?php echo $tanggal ?>"
                ...
                <div class="table-responsive">
                  <table id="mytable" class="table table-bordered table-striped">
                    <thead>
                      <tr>
                        <th width="10">No</th>
                        <th width="350">IMEI / Nama Produk</th>
          ...
          success: function (res) {
            if (res.status == "ok") {
              response($.map(res.data, function (value, key) {
                return {
                  value: value.value,
                  label: value.barcode + ' / ' + value.label,
                };
              }));
            } else {
              alert("Error");
    ...
    function _generate_table(data) {
      ...
      for (var item in data) {
        ...
        html += `
              <tr>
                <td>`+ no + `</td>
                <td>`+ data[item].barcode + ` / ` + data[item].nama_produk + `<p><b>Deskripsi :</b></p><p>` + data[item].deskripsi + `<p></td>
                <td style="padding:0px;"><input type="text" readonly class="form-control input-number stok_kembali" name="stok_kembali[`+ data[item].id_produk + `]" value="` + data[item].jumlah + `" data-id="` + data[item].id + `" style="text-align:center;"></td>
