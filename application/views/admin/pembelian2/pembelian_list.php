      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="row" style="margin-bottom: 10px">
            <div class="col-md-4">
              <?php echo anchor(site_url('admin/pembelian_produk_langsung/create'), 'Pembelian Baru', 'class="btn btn-primary "'); ?>
            </div>
            ...
            <tbody>
              ...
              foreach ($data_faktur as $df):
                ...
                <tr>
                  ...
                  <td>
                    <?php echo $df->tgl_faktur ?>
                  </td>
                  <td>
                    <?php echo $df->no_faktur ?>
                    <?php if (!empty($df->id_retur)) { ?>
                      <br>
                      <a href="<?php echo base_url() ?>admin/retur/read/<?php echo $df->id_retur ?>"><span class="red">Ada
                          retur</span></a>
                    <?php } ?>
                  </td>
                  <td>
                    <?php echo $df->nama_supplier ?>
                  </td>
                  <td>
                    <?php
                    $pmb = $df->pembayaran;
                    if ($pmb == 1) {
                      ...
                      $bank = $this->db->select('b.*')
                        ->from('pil_bank b')
                        ->where('b.id', $df->id_bank)
                        ->get()
                        ->row();
                      if ($bank != null) {
                        echo '<label for="" class="label label-success">TRANSFER ' . $bank->bank . '</label>';

                      } else {

                        echo '<label for="" class="label label-success">TRANSFER</label>';
                      }

                    } else {
                      echo '<label for="" class="label label-">-</label>';
                  ...
                  <td>
                    <label for="" class="label label-warning">
                      <?php echo $df->jml_produk; ?> Produk
                    </label>
                    <label for="" class="label label-primary">
                      <?php echo $df->jml_item; ?> Pcs
                    </label>
                  </td>
                  <td>
                    <?php echo number_format($df->total, 0, ',', '.') ?>
                  </td>
                  <td class="text-center">
                    <a href="<?php echo base_url() ?>admin/pembelian/read/<?php echo $df->id_faktur ?>"
                      class="btn btn-success btn-xs"><i class="ace-icon fa fa-check"></i></a>
                    <a href="<?php echo base_url() ?>admin/pembelian/update/<?php echo $df->id_faktur ?>"
                      class="btn btn-info btn-xs"><i class="ace-icon fa fa-pencil"></i></a>
                  </td>
                </tr>
                <?php $no++; endforeach; ?>
            </tbody>
